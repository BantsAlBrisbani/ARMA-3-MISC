while {true} do {
    ...
    if (!alive player || {!alive _owner}) exitWith {
        ["Disconnected from handset.", true] call TAG_Handset_fnc_releaseHandset;
    };

    if ((player distance _owner) > TAG_HANDSET_MAX_CABLE_DISTANCE) exitWith {
        ["Disconnected from handset: out of range.", true] call TAG_Handset_fnc_releaseHandset;
    };
    ...
    if (vehicle _owner != _owner) exitWith {
        ["Disconnected from handset.", true] call TAG_Handset_fnc_releaseHandset;
    };
    ...
    if (!_stillExists) exitWith {
        ["Disconnected from handset: radio unavailable.", true] call TAG_Handset_fnc_releaseHandset;
    };
